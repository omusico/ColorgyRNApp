  ...
  public void receiveCommand(ReactTableCreateCoursePageLayout view, int commandType, @Nullable ReadableArray args) {
    ...
    switch (commandType) {
      case COMMAND_SET_LAYOUT: {
        if (null != view.backgroundIndex) {
          LinearLayout.LayoutParams blp = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
          View child = (View) view.mBackground.getChildAt(view.backgroundIndex);
          child.setLayoutParams(blp);

          // ViewGroup oldParent = (ViewGroup) child.getParent();

          // oldParent.removeView(child);
          // view.mBackground.addView(child);
        }
        if (null != view.scrollviewIndex) {
          LinearLayout.LayoutParams svlp = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
          view.mContent.getChildAt(view.scrollviewIndex).setLayoutParams(svlp);
          view.mContent.getChildAt(view.scrollviewIndex).postInvalidate();
        }
        view.postInvalidate();
        return;
      }
